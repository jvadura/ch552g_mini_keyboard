...
__xdata uint8_t HIDConsumer[8] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0}; // 4x 16-bit consumer codes (matches REPORT_COUNT=4)
...
uint8_t Consumer_press(__data uint16_t key) {
  // Array implementation matching CH552-Macropad reference
  // HIDConsumer buffer: 8 bytes = 4 slots of 16-bit consumer codes
  // Each slot: [low_byte, high_byte]
  __data uint8_t i;

  // Check if key is already present in report (avoid duplicates)
  for(i = 0; i < 8; i += 2) {
    if((HIDConsumer[i] == (key & 0xFF)) && (HIDConsumer[i+1] == (key >> 8))) {
      return 1; // Already pressed, don't send again
    }
  }

  // Find an empty slot and insert key
  for(i = 0; i < 8; i += 2) {
    if((HIDConsumer[i] == 0) && (HIDConsumer[i+1] == 0)) {  // Empty slot?
      HIDConsumer[i]   = key & 0xFF;        // Low byte
      HIDConsumer[i+1] = (key >> 8) & 0xFF; // High byte
      USB_EP1_send(3);                      // Send report
      return 1;
    }
  }
...
uint8_t Consumer_release(__data uint16_t key) {
  // Find and release the specific key
  __data uint8_t i;
  for(i = 0; i < 8; i += 2) {
    if((HIDConsumer[i] == (key & 0xFF)) && (HIDConsumer[i+1] == (key >> 8))) {
      HIDConsumer[i] = 0;
...
uint8_t Consumer_write(__data uint16_t key) {
  // Press and release in one call (perfect for encoder)
  Consumer_press(key);
  delayMicroseconds(10000); // 10ms delay between press and release
  Consumer_release(key);
  return 1;
}
